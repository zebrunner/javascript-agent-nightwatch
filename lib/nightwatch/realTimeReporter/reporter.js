...
class ZebrunnerReporter {
  ...
  #subscribeServerEvents = (server) => {
    ...
    server.on(REPORTING_EVENTS.START_TEST, async (message) => {
      ...
      this.#storage.addTestStartPromise(this.#startTest(message));
    });
    ...
    server.on(REPORTING_EVENTS.ATTACH_TEST_LABELS, async (message) => {
      await Promise.all(this.#storage.testStartPromises);
  ...
  #finishTest = async (test) => {
    ...
    this.#storage.addTestFinishPromise(testCaseFinishPromise);
  };
  ...
  finishTestRun = async () => {
    await Promise.all(this.#storage.testFinishPromises);
    ...
    if (this.#storage.runId) {
      ...
      await Promise.all([
        ...
        ...this.#storage.testArtifactsFinishPromises,
      ]);
  ...
  #attachTestRunLabels = async (labels) => {
    if (this.#storage.runId && labels && labels.length !== 0) {
      ...
      this.#storage.addTestArtifactFinishPromise(attachLabelsPromise);
    }
  };
  ...
  #attachTestRunArtifactReferences = async (references) => {
    if (this.#storage.runId && references && references.length !== 0) {
      ...
      this.#storage.addTestArtifactFinishPromise(attachReferencesPromise);
    }
  };
  ...
  #uploadTestRunArtifact = async (file) => {
    if (this.#storage.runId && file) {
      ...
      this.#storage.addTestArtifactFinishPromise(uploadArtifactPromise);
    }
  };
  ...
  #setTestMaintainer = async (message) => {
    ...
    if (testId && maintainer) {
      ...
      const testUpdateBody = getTestUpdate(maintainer);
      const headers = await this.#getHeadersWithAuth(jsonHeaders);

      const testUpdatePromise = this.#httpClient.callPatch(url, testUpdateBody, headers);
