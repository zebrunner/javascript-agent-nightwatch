...
const { getNightwatchSkipTestInfo } = require('../../object-transformer');
...
class Reporter {
  ...
  #startTestSession = async (browserConfig) => {
    if (this.runId) {
      const url = API_URLS.URL_START_SESSION.replace('${testRunId}', this.runId);

      const sessionStartBody = getTestSessionStart(browserConfig);
      const headers = await this.#getHeadersWithAuth(jsonHeaders);

      this.#httpClient.callPost(url, sessionStartBody, headers).then((sessionStartResponse) => {
        this.#testSessionsMap.set(this.runId, {
          zbrSessionId: sessionStartResponse.data.id,
        });
      });
    }
  };
  ...
  #finishTestSession = async () => {
    if (this.runId) {
      const sessionId = this.#testSessionsMap.get(this.runId).zbrSessionId;
      const url = API_URLS.URL_FINISH_SESSION.replace('${testRunId}', this.runId).replace('${testSessionId}', sessionId);

      const testIds = Array.from(this.#testsMap.values()).map((i) => i.zbrTestId);
      const sessionEndBody = getTestSessionEnd(testIds);

      const headers = await this.#getHeadersWithAuth(jsonHeaders);

      const sessionEndResponse = this.#httpClient.callPut(url, sessionEndBody, headers);
      this.#testSessionsMap.get(this.runId).promiseSessionFinish = sessionEndResponse;
    }
  };

  #startSuite = (browserConfig) => {
    this.#startTestSession(browserConfig);
  };

  #finishSuite = (test) => {
    this.#finishTestSession();
    ...
    if (test.steps.length !== 0) {
      test.steps.forEach(async (step) => {
        const skipTest = getNightwatchSkipTestInfo(step, test.testFileName);
  ...
  finishTestRun = async () => {
    if (this.runId) {
      ...
      const finishSessionPromisesArr = Array.from(this.#testsMap.values()).map((i) => i.promiseSessionFinish);
