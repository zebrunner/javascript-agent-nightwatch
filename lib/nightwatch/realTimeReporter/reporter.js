...
const { parseNightwatchSkipTestInfo } = require('../../object-transformer');
...
class Reporter {
  ...
  #subscribeServerEvents = (server) => {
    server.on(REPORTING_EVENTS.START_SUITE, this.#startSuite);
    server.on(REPORTING_EVENTS.FINISH_SUITE, this.#finishSuite);
  ...
  #unsubscribeServerEvents = (server) => {
    server.off(REPORTING_EVENTS.START_SUITE, '*');
    server.off(REPORTING_EVENTS.FINISH_SUITE, '*');
  ...
  #startTest = async (test) => {
    if (this.#runId) {
      ...
      if (test.sessionId && !sessionId) {
        console.log(`No test session found, started a new one for ${test.sessionId}`);
  ...
  #finishTest = async (test) => {
    ...
    currentTest.promiseStart.then(async ({ data }) => {
      ...
      if (test.status !== testStatuses.SKIPPED) {
        await this.#linkTestAndTestSession(test);

        if (test.sessionId === null) {
          console.log('Browser session is killed, test session will be finished');
          await this.#finishTestSession();
        }

        if (test.assertions) {
          await this.#processAssertions(currentTest.zbrTestId, test);
        }
      }
  ...
  #getSessionId = async () => {
    if (this.#sessionId) {
      const { data } = await this.#sessionId;
  ...
  #startTestSession = async (test) => {
    if (this.#runId) {
      ...
      this.#sessionId = sessionStartResponse;
    }
  };
  ...
  #linkTestAndTestSession = async (test) => {
    ...
    if (this.#runId && sessionId) {
      ...
      this.#testsMap.get(test.uniqueId).zbrSessionId = sessionId;
  ...
  #finishSuite = async (testSuite) => {
    ...
    if (testSuite.steps.length !== 0) {
      testSuite.steps.forEach(async (step) => {
        const skipTest = parseNightwatchSkipTestInfo(step, testSuite.testFileName);
