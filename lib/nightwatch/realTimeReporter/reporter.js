...
class Reporter {
  ...
  #finishTest = async (test) => {
    if (this.#testsMap.get(test.uniqueId)) {
      ...
      this.#processAssertions(testId, test);
  ...
  #processAssertions = async (testId, test) => {
    const assertionsArr = test.assertions.map((obj, index) => ({ ...obj, timestamp: Date.now() + index }));
    const screenshotsArr = assertionsArr.filter((obj) => obj.screenshots).map(({ screenshots, timestamp }) => ({ screenshots, timestamp }));

    this.#sendLogs(testId, assertionsArr);

    screenshotsArr.forEach((screenshot) => {
      this.#sendScreenshot(testId, getScreenshotByName(screenshot.screenshots[0]), screenshot.timestamp);
    });
  };
  ...
  #sendScreenshot = async (testId, imgPath, timestamp) => {
    const url = API_URLS.URL_SEND_SCREENSHOT.replace('${testRunId}', this.runId).replace('${testId}', testId);
  ...
  finishTestRun = async () => {
    ...
    if (this.runId) {
      ...
      const screenshotsReadFilePromisesArr = Array.from(this.#screenshotsMap.values()).map((i) => i.promiseScreenshotRead);
      ...
      if (screenshotsReadFilePromisesArr.length) {
        const screenshotsSendPromisesArr = Array.from(this.#screenshotsMap.values()).map((i) => i.promiseScreenshotSend);
