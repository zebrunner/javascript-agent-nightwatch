...
class ZebrunnerReporter {
  ...
  #startTest = async (test, processTestSession = true) => {
    if (this.#runId) {
      ...
      this.#testsMap.set(this.#getTestUniqueId(test), {
  ...
  */
  ...
  #finishTest = async (test) => {
    let currentTest = this.#testsMap.get(this.#getTestUniqueId(test));

    if (!currentTest) {
      await this.#startTest(test, false);
      currentTest = this.#testsMap.get(this.#getTestUniqueId(test));
    }
