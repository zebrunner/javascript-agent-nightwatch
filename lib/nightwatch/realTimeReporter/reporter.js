...
class ZebrunnerReporter {
  ...
  constructor(config) {
    ...
    this.#runId = null;
    this.#currentSessionsMap = new Map();
  ...
  #getSession = async (test) => {
    if (this.#currentSessionsMap.has(test.moduleUniqueId)) {
      const { data } = await this.#currentSessionsMap.get(test.moduleUniqueId).promiseStartSession;
  ...
  #startTest = async (test) => {
    if (this.#runId) {
      ...
      await this.#processTestSession(test);
    }
  };
  ...
  */
  ...
  #startTestSession = async (test) => {
    if (this.#runId) {
      ...
      this.#currentSessionsMap.set(test.moduleUniqueId, {
        promiseStartSession: sessionStartResponse,
      });
    }
  };
  ...
  #finishTestSession = async (test) => {
    ...
    if (this.#runId && sessionId) {
      ...
      this.#currentSessionsMap.delete(test.moduleUniqueId);
    }
  };
