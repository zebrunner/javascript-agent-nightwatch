...
const { parseNightwatchSkipTestInfo, getNightwatchTestInfoAsOneTest, combineTestCaseResults } = require('../../object-transformer');
...
class ZebrunnerReporter {
  ...
  #getTestUniqueId = (test) => (this.#testSuitesMap.get(test.moduleUniqueId).trackAsOneZbrTest ? test.moduleUniqueId : test.uniqueId);
  ...
  #startTest = async (test) => {
    if (this.#runId) {
      ...
      this.#testsMap.set(testInfo.uniqueId, {
  ...
  */
  ...
  #finishTest = async (test) => {
    const currentTest = this.#testsMap.get(this.#getTestUniqueId(test));
    const testInfo = this.#testSuitesMap.get(test.moduleUniqueId).trackAsOneZbrTest ? combineTestCaseResults(test) : test;

    const finishTestPromise = currentTest.promiseStart.then(async ({ data }) => {
      currentTest.zbrTestId = data.id;
      currentTest.startedAt = data.startedAt;
      ...
      if (testInfo.status !== testStatuses.SKIPPED) {
        ...
        if (testInfo.assertions) {
          await this.#processAssertions(currentTest.zbrTestId, currentTest.startedAt, testInfo);
        }
      }
      ...
      const url = API_URLS.URL_FINISH_TEST.replace('${testRunId}', this.#runId).replace(
        '${testId}',
        currentTest.zbrTestId,
      );
      const headers = await this.#getHeadersWithAuth(jsonHeaders);

      const testEndResponse = this.#httpClient.callPut(url, testEndBody, headers);
      this.#testsMap.get(this.#getTestUniqueId(testInfo)).promiseFinish = testEndResponse;
    });

    this.#finishPromises.push(finishTestPromise);
  };
  ...
  #startTestSession = async (test) => {
    if (this.#runId) {
      const url = API_URLS.URL_START_SESSION.replace('${testRunId}', this.#runId);
  ...
  #linkTestAndTestSession = async (test) => {
    ...
    if (this.#runId && sessionId) {
      const url = API_URLS.URL_UPDATE_SESSION.replace('${testRunId}', this.#runId).replace('${testSessionId}', sessionId);

      const testId = this.#testsMap.get(this.#getTestUniqueId(test)).zbrTestId;
      const sessionUpdateBody = getTestSessionUpdate([testId]);
      const headers = await this.#getHeadersWithAuth(jsonHeaders);

      const sessionUpdateResponse = this.#httpClient.callPut(url, sessionUpdateBody, headers);
      this.#testsMap.get(this.#getTestUniqueId(test)).zbrSessionId = sessionId;
      this.#testSessionsMap.set(sessionId, {
        promiseLinkTest: sessionUpdateResponse,
      });
    }
  };
  ...
  #finishTestSession = async (test) => {
    ...
    if (this.#runId && sessionId) {
      const linkSessionPromisesArr = Array.from(this.#testSessionsMap.values()).map((i) => i.promiseLinkTest);
      await Promise.all(linkSessionPromisesArr);

      const url = API_URLS.URL_FINISH_SESSION.replace('${testRunId}', this.#runId).replace('${testSessionId}', sessionId);
  ...
  #finishTestSuite = async (testSuite) => {
    ...
    if (!this.#testSuitesMap.get(testSuite.moduleUniqueId).trackAsOneZbrTest && testSuite.steps.length !== 0) {
      testSuite.steps.forEach(async (step) => {
        const skipTest = parseNightwatchSkipTestInfo(testSuite, step);
        await this.#startTest(skipTest);
        this.#finishTest(skipTest);
      });
    }
  };
  ...
  #initializeRunContext = async () => {
    ...
    if (runContext) {
      const headers = await this.#getHeadersWithAuth(jsonHeaders);
      const { data } = await this.#httpClient.callPost(API_URLS.URL_EXCHANGE_RUN_CONTEXT, runContext, headers);
  ...
  finishTestRun = async () => {
    await Promise.all(this.#finishPromises);

    if (this.#runId) {
      const startPromisesArr = Array.from(this.#testsMap.values()).map((i) => i.promiseStart);
      await Promise.all(startPromisesArr);

      const finishPromisesArr = Array.from(this.#testsMap.values()).map((i) => i.promiseFinish);
      const finishSessionPromisesArr = Array.from(this.#testSessionsMap.values()).map((i) => i.promiseFinish);
