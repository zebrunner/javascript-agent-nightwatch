...
class Reporter {
  ...
  #processAssertions = async (testId, test) => {
    this.#sendLogs(testId, test.assertions);
  };
  ...
  #sendScreenshot = async (testId, imgPath) => {
    ...
    if (imgPath?.length < 255) {
      const screenshotReadPromise = new Promise((resolve) => {
        fs.readFile(imgPath, (err, data) => {
          ...
          this.#screenshotsMap.get(testId).screenshotSendPromise = response;
  ...
  finishTestRun = async () => {
    ...
    if (this.runId) {
      ...
      await Promise.all([...finishPromisesArr, ...logsPromisesArr, ...screenshotsPromisesArr]);
