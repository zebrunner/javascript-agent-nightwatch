...
const {
  getScreenshotByName, generateMultipartBoundary, isBlankString, isEmptyObject,
} = require('../../utils');
const {
...
class ZebrunnerReporter {
  ...
  #finishTest = async (test) => {
    ...
    const testCaseFinishPromise = currentTest.startPromise.then(async ({ data }) => {
      ...
      if (testCases.length !== 0) {
        ...
        const updatedTestCases = this.#setDefaultStatusIfActualNotProvided(testCases, defaultTestCaseStatus);
  ...
  #upsertTestTestCases = async (testId, testCases) => {
    if (this.#storage.runId && testId) {
      const url = API_URLS.URL_UPSERT_TEST_TEST_CASES.replace('${testRunId}', this.#storage.runId).replace('${testId}', testId);
  ...
  #saveTcmConfigs = async () => {
    ...
    if (this.#storage.runId && !isEmptyObject(updateTcmConfigsBody)) {
      ...
      const headers = await this.#getHeadersWithAuth(jsonHeaders);
      const updateTcmConfigsPromise = this.#httpClient.callPatch(url, updateTcmConfigsBody, headers);
