...
class ZebrunnerReporter {
  ...
  #subscribeServerEvents = (server) => {
    ...
    server.on(REPORTING_EVENTS.ATTACH_TEST_LABELS, this.#attachTestLabels);
    server.on(REPORTING_EVENTS.ATTACH_TEST_ARTIFACT_REFERENCES, this.#attachTestArtifactReferences);
  };
  ...
  #attachTestLabels = async (message) => {
    ...
    const testId = this.#storage.getTestByUniqueId(this.#getTestUniqueId(message.test)).zbrId;
