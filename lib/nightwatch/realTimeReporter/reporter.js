...
class ZebrunnerReporter {
  ...
  #subscribeServerEvents = (server) => {
    ...
    server.on(
      REPORTING_EVENTS.ATTACH_TEST_RUN_ARTIFACT_REFERENCES,
      this.#attachTestRunArtifactReferences,
    );
    ...
    server.on(REPORTING_EVENTS.ADD_TEST_CASES, async (message) => {
      await this.#waitForTestStartPromises();
      this.#addTestCases(message);
    });
    server.on(REPORTING_EVENTS.SAVE_TEST_SCREENSHOT, async (message) => {
      await this.#waitForTestStartPromises();
  ...
  #unsubscribeServerEvents = (server) => {
    ...
    server.off(REPORTING_EVENTS.ADD_TEST_CASES, '*');
    server.off(REPORTING_EVENTS.SAVE_TEST_SCREENSHOT, '*');
  };
  ...
  #getSession = async (test) => {
    if (this.#storage.getTestSuiteByUniqueId(test.moduleUniqueId).sessionStartPromise) {
      const { data } = await this.#storage.getTestSuiteByUniqueId(test.moduleUniqueId)
        .sessionStartPromise;
  ...
  #skipTest = async (test, title) => {
    const { testName } = this.#storage.getTestSuiteByUniqueId(test.moduleUniqueId);
    const skipTest = parseNightwatchSkipTestInfo(test.testFileName, title, testName);
  ...
  #startTest = async (test, processTestSession = true) => {
    if (this.#storage.runId) {
      ...
      this.#storage.setTestSuiteTrackAsOneZbrTest(
        testInfo.moduleUniqueId,
        testInfo.trackAsOneZbrTest,
      );
      this.#storage.setTestSuiteName(testInfo.moduleUniqueId, testInfo.testName);
  ...
  #finishTest = async (test) => {
    ...
    const testCaseFinishPromise = currentTest.startPromise.then(async ({ data }) => {
      ...
      const testCases = this.#storage.getTestTestCases(testUniqueId);

      if (testCases.length !== 0) {
        // update results for TCM test cases
        const defaultTestCaseStatus = this.#getDefaultTestCaseStatus(testInfo.status);
        const updatedTestCases = this.#setDefaultStatusIfActualNotProvided(
          testCases,
          defaultTestCaseStatus,
        );
        await this.#upsertTestTestCases(data.id, updatedTestCases);
      }
  ...
  /** Called when test is finished to sync automation test results with linked TCM test cases */
  #upsertTestTestCases = async (testId, testCases) => {
    if (this.#storage.runId && testId) {
      const url = API_URLS.URL_UPSERT_TEST_TEST_CASES.replace(
        '${testRunId}',
        this.#storage.runId,
      ).replace('${testId}', testId);
      const upsertTestCasesBody = getUpsertTestCases(testCases);
      const headers = await this.#getHeadersWithAuth(jsonHeaders);
      const upsertTestCasesPromise = this.#httpClient.callPost(url, upsertTestCasesBody, headers);
  ...
  /**
   * Called when test is finished and its results include assertions done in this test.
  ...
  #finishTestSuite = async (testSuite) => {
    ...
    if (
      !this.#storage.getTestSuiteByUniqueId(testSuite.moduleUniqueId).trackAsOneZbrTest
      && testSuite.steps
    ) {
  ...
  #saveTcmConfigs = async () => {
    ...
    if (this.#storage.runId && !isEmptyObject(updateTcmConfigsBody)) {
      const url = API_URLS.URL_UPDATE_TCM_CONFIGS.replace('${testRunId}', this.#storage.runId);
      const headers = await this.#getHeadersWithAuth(jsonHeaders);
      const updateTcmConfigsPromise = this.#httpClient.callPatch(
        url,
        updateTcmConfigsBody,
        headers,
      );
  ...
  #uploadTestRunArtifact = async (file) => {
    if (this.#storage.runId && file) {
      const url = API_URLS.URL_UPLOAD_TEST_RUN_ARTIFACT.replace(
        '${testRunId}',
        this.#storage.runId,
      );
      ...
      const uploadArtifactPromise = this.#httpClient.callPost(
        url,
        attachArtifactMultipartBody,
        headers,
      );
  ...
  #attachTestLabels = async (message) => {
    ...
    if (testId && labels && labels.length !== 0) {
      const url = API_URLS.URL_ATTACH_TEST_LABELS.replace(
        '${testRunId}',
        this.#storage.runId,
      ).replace('${testId}', testId);
      const attachLabelsBody = getAttachLabels(labels);
  ...
  #uploadTestArtifact = async (message) => {
    ...
    if (testId && file) {
      const url = API_URLS.URL_UPLOAD_TEST_ARTIFACT.replace(
        '${testRunId}',
        this.#storage.runId,
      ).replace('${testId}', testId);
      const boundary = generateMultipartBoundary();
  ...
  #setTestMaintainer = async (message) => {
    ...
    if (testId && maintainer) {
      const url = API_URLS.URL_UPDATE_TEST.replace('${testRunId}', this.#storage.runId).replace(
        '${testId}',
        testId,
      );
  ...
  #revertTestRegistration = async (message) => {
    ...
    if (testId) {
      const url = API_URLS.URL_REVERT_TEST_REGISTRATION.replace(
        '${testRunId}',
        this.#storage.runId,
      ).replace('${testId}', testId);
      const headers = await this.#getHeadersWithAuth(jsonHeaders);
  ...
  #addTestCases = async (message) => {
    const { test, testCase } = message;
    const testId = await this.#storage.getTestZbrId(this.#getTestUniqueId(test));

    if (testId) {
      this.#storage.addTestTestCase(this.#getTestUniqueId(test), testCase);
    }
  };

  #saveScreenshot = async (message) => {
    const { test, filePath, timestamp } = message;
    const testId = await this.#storage.getTestZbrId(this.#getTestUniqueId(test));
    const file = await waitForFileExists(filePath);

    if (testId && file) {
      this.#sendScreenshot(testId, filePath, timestamp);
    }
  };
}
