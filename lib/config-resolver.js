const { summarySendingPolicies } = require('./constants');
...
class ConfigResolver {
  ...
  getReportingRunDisplayName() {
    return this.getConfig('REPORTING_LAUNCH_DISPLAY_NAME', 'launch.displayName', DEFAULT_TEST_RUN_NAME);
  }

  getReportingRunBuild() {
    return this.getConfig('REPORTING_LAUNCH_BUILD', 'launch.build');
  }

  getReportingRunEnvironment() {
    return this.getConfig('REPORTING_LAUNCH_ENVIRONMENT', 'launch.environment');
  }

  getReportingRunLocale() {
    return this.getConfig('REPORTING_LAUNCH_LOCALE', 'launch.locale');
  }

  getReportingTreatSkipsAsFailures() {
    return this.getConfig('REPORTING_LAUNCH_TREAT_SKIPS_AS_FAILURES', 'launch.treatSkipsAsFailures', true);
  }

  getReportingRunLabels() {
    const labels = this.getConfig(null, 'launch.labels') || {};

    return Object.keys(labels).filter((key) => key && key.trim()).map((key) => ({ key, value: labels[key] }));
  }

  getReportingRunArtifactReferences() {
    const artifactReferences = this.getConfig(null, 'launch.artifactReferences') || {};
  ...
  getReportingNotificationSummarySendingPolicy() {
    const sendingPolicyConfig = this.getConfig('REPORTING_NOTIFICATION_SUMMARY_SENDING_POLICY', 'notifications.summarySendingPolicy');
    if (!sendingPolicyConfig) {
      return undefined;
    }

    return Object.keys(summarySendingPolicies).find((sendingPolicy) => sendingPolicy.toLowerCase() === sendingPolicyConfig.toLowerCase());
  }
