...
const parseNightwatchTestInfo = (browser, testName) => {
  ...
  return {
    ...
    testName: testName || getFileNameFromPath(test.module),
    trackAsOneZbrTest: test.name === '',
  };
};

const parseNightwatchSkipTestInfo = (testFileName, title, testName) => ({
  title,
  fullTitle: testName ? `${testName} - ${title}` : `${getFileNameFromPath(testFileName)} - ${title}`,
  status: testStatuses.SKIPPED,
  testFileName,
  uniqueId: Buffer.from(`${testFileName}-${title}`, 'utf-8').toString('base64'),
  moduleUniqueId: Buffer.from(testFileName, 'utf-8').toString('base64'),
  testName: testName || getFileNameFromPath(testFileName),
...
const combineTestCaseResults = (test) => {
  ...
  if (test.testcases) {
    Object.keys(test.testcases).forEach((key) => {
      ...
      if (test.testcases[key].errors !== 0) {
        testCaseResults.stackTrace = test.testcases[key].stackTrace;
      }
    });
