...
const parseNightwatchTestInfo = (test) => {
...
const getNightwatchSkipTestInfo = (name, module) => ({
  title: name,
  fullTitle: `${getFileNameFromPath(module)} - ${name}`,
  status: testStatuses.SKIPPED,
  testFileName: module,
  uniqueId: Buffer.from(`${module}-${name}`, 'utf-8').toString('base64'),
});

const parseNightwatchBrowserConfig = (browser) => ({
...
module.exports = {
  ...
  getNightwatchSkipTestInfo,
  parseNightwatchBrowserConfig,
};
